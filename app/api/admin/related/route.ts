...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const saved = await db.relatedPropertiesSection.upsert({
    ...
    return NextResponse.json({
      message: "Berhasil menyimpan Related section",
      data: saved,
    });
  } catch (e) {
    console.error("[ADMIN_RELATED_POST]", e);
    return NextResponse.json(
      { message: "Gagal menyimpan Related section" },
      { status: 500 }
    );
  }
};
